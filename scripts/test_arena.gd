...
func _ready():
	...
	if simulation_manager == null:
		push_error("SimulationManager node not found! The GDExtension may not be loaded properly.")
		print("ERROR: Make sure the extension is built and the .gdextension file is configured correctly.")
		return

	if agent == null:
		push_error("Agent node not found! The GDExtension may not be loaded properly.")
		return
...
func _process(_delta):
	if simulation_manager == null or agent == null:
		label.text = "Agent Arena Test Scene (ERROR)
...
func test_agent():
	if agent == null:
		push_error("Cannot test agent - agent is null")
		return
